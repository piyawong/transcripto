...
ต้องมี: Docker, Rust (stable), Node 24, ffmpeg, Google Chrome (สำหรับ e2e)
...
|---|---|
| อัปโหลดไฟล์ | browser ส่งไฟล์ด้วย `PUT /api/jobs/{id}/file` → route handler ของ Next ส่งต่อแบบ stream → API เขียนไฟล์ชั่วคราว เช็กว่าขนาดครบ → เก็บใน MinIO `jobs/<id>/source.<ext>` แล้วจึงเข้าคิว |
